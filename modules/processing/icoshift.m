...
if (matVersion), narginchk(2,7);
elseif (nargin < 2), error('icoShift:tooFew','Not enough input arguments');
elseif (nargin > 7), error('icoShift:tooMany','Too many input arguments');
...
% Scal
useScal = false;
if (nargin < 7), Scal = [];  else useScal = options.useScal; end
...
% Target
[xT,pars] = getTarget(xT,xP,pars,options,multiplicador);
...
if (pars.COShift)
    ...
    [xP,~,wint] = icoshift(Target,xP,'whole',options.maxPreCOShift,[0 1 0],[],multiplicador);
    if (ischar(pars.note) && ~isempty(pars.note)), xT = getTarget(pars.note,xP,pars,options, multiplicador); end
    if (options.show) 
